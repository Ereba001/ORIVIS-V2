...
export default function SignIn({ onSuccess, variant, compact }: SignInProps) {
  ...
  const formContent = (
    <>
      ...
      <div className="mt-3 flex flex-col items-center gap-1.5">
        <button
          type="button"
          onClick={() => navigate(variant === "platform" ? "/forgot-password" : "/org/forgot-password")}
  ...
  if (compact) {
    return (
      <div className="relative">
        {showLoading && <LoadingOverlay messages={["Authenticating your credentials...", "Connecting to secure server...", "Loading your dashboard..."]} />}
        {formContent}
      </div>
    );
  }
