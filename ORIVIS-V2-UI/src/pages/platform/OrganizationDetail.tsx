...
import {
  ...
  HeartPulse, CheckCircle2, AlertTriangle, XCircle, MessageSquare,
...
const MOCK_EVENT_COUNTS = [
  { label: "Draft", count: 2, variant: "neutral" as const },
  { label: "Pending Review", count: 1, variant: "warning" as const },
  { label: "Published", count: 3, variant: "info" as const },
  { label: "Live", count: 1, variant: "success" as const },
  { label: "Completed", count: 5, variant: "success" as const },
  { label: "Archived", count: 1, variant: "neutral" as const },
]
...
function deriveEmail(name: string, slug: string): string {
...
export default function OrganizationDetail() {
  ...
  const { enterInspection } = usePlatformGovernance()
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 glass-card rounded-2xl p-5 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <ProfileField icon={Mail} label="Email" value={deriveEmail(org.organizationName, org.slug)} />
              <ProfileField icon={Phone} label="Phone" value="+234 800 000 0000" />
            ...
            <div className="space-y-3">
              ...
              <div className="w-full bg-brand-surface rounded-full h-1.5 overflow-hidden">
                <div
                  ...
                  style={{ width: `${(org.storageUsed / org.storageTotal) * 100}%` }}
                />
              </div>
            </div>
          </div>
    ...
    return (
      <div className="max-w-2xl glass-card rounded-2xl p-6 space-y-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          ...
          <SubRow icon={Users} label="Max Voters" value={sub.maxVoters.toLocaleString()} />
  ...
  const renderHealthTab = () => {
    const pct = Math.round((org.storageUsed / org.storageTotal) * 100)
    const HealthIcon = HEALTH_ICONS[org.workspaceStatus] || CheckCircle2
    return (
  ...
  const renderEventsTab = () => (
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
      {MOCK_EVENT_COUNTS.map((evt) => (
        ...
        >
          <p className="text-3xl font-bold font-mono text-brand-text-primary">{evt.count}</p>
          <p className="text-[10px] font-mono uppercase tracking-wider text-brand-text-muted mt-2">{evt.label}</p>
          <div className="mt-3">
            <StatusPill status={evt.label} variant={evt.variant} />
          </div>
        </motion.div>
      ))}
    </div>
  ...
  const renderMembersTab = () => {
    const counts = [
      { label: "Members", value: org.members.toLocaleString(), icon: Users, color: "text-brand-gold" },
      { label: "Admins", value: org.admins.toLocaleString(), icon: Shield, color: "text-blue-400" },
      { label: "Invitations", value: memberships.filter((m) => m.status === "Pending").length.toString() || "2", icon: MessageSquare, color: "text-amber-400" },
    ]
    return (
      <div className="space-y-6">
      ...
      {invoices.length === 0 ? (
        ...
        <div className="glass-card rounded-2xl overflow-hidden">
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-brand-border text-[9px] font-mono uppercase tracking-wider text-brand-text-muted">
            ...
            <tbody>
              {invoices.map((inv) => (
                <tr key={inv.id} className="border-b border-brand-border last:border-0">
                  <td className="p-4 text-xs text-brand-text-primary">{inv.date}</td>
                  <td className="p-4 text-xs font-mono text-brand-text-primary">{inv.id}</td>
                  <td className="p-4 text-xs text-brand-text-muted">{inv.plan}</td>
                  <td className="p-4 text-xs font-mono text-brand-text-primary">{inv.amount}</td>
                  <td className="p-4">
            ...
            <tbody>
              {auditLogs.map((log) => (
                <tr key={log.id} className="border-b border-brand-border last:border-0">
                  <td className="p-4 text-xs text-brand-text-primary">{log.action}</td>
                  <td className="p-4 text-xs font-mono text-brand-text-muted">{log.user}</td>
                  <td className="p-4">
                  ...
                  <td className="p-4 text-[10px] font-mono text-brand-text-muted">{log.timestamp}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6">
            ...
            <div className="flex flex-wrap items-center gap-2">
              <motion.button
                ...
                whileTap={{ scale: 0.98 }}
                onClick={() => {
                  enterInspection(org.organizationId, org.organizationName)
                }}
              ...
              >
                ...
                <span>Enter Workspace Inspection</span>
              </motion.button>
