...
import {
  ...
  User, Settings, CreditCard, LogIn, Vote, Shield, Users,
} from 'lucide-react'
...
export default function OrgAuditLogs() {
  ...
  const pagedEntries = allGroupedEntries.slice((page - 1), page)
  ...
  return (
    ...
    <div className="space-y-6">
      ...
      {/* === 3-TAB NAVIGATION === */}
      <div className="flex items-center gap-1 p-1 rounded-2xl bg-brand-surface-elevated/30 w-fit">
        {TABS.map((t) => {
          const TabIcon = t.icon
          const isActive = tab === t.key
          return (
            <button key={t.key} onClick={() => { setTab(t.key); setPage(1); setSearch(''); setSeverityFilter('all') }}
              className={`flex items-center gap-2 px-4 py-2 rounded-xl text-[10px] font-mono font-bold uppercase tracking-wider transition-all ${
                isActive
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <div className="relative flex-1 max-w-md">
          ...
          <input value={search} onChange={(e) => { setSearch(e.target.value); setPage(1) }} placeholder="Search audit log..."
          ...
          {pagedEntries.map(({ date, events }) => (
            <div key={date}>
              <div className="flex items-center gap-2 mb-3">
                ...
                <h3 className="text-xs font-bold uppercase tracking-wider" style={{ color: pColor }}>{date}</h3>
                <span className="text-[9px] font-mono text-brand-text-muted">({events.length} event{events.length !== 1 ? 's' : ''})</span>
              </div>
                          ...
                          <div className="flex items-center gap-2.5 min-w-0">
                            ...
                            <div className="min-w-0">
                              ...
                              <div className="flex items-center gap-2 mt-0.5">
                                <span className="text-[9px] font-mono text-brand-text-muted">{event.user}</span>
                                <span className="text-brand-text-disabled">·</span>
                                <span className="text-[9px] font-mono text-brand-text-muted">{formatTimeAgo(event.timestamp)}</span>
                                {event.ipAddress !== 'system' && (
                                  <>
                                    <span className="text-brand-text-disabled">·</span>
                                    <span className="text-[9px] font-mono text-brand-text-muted">{event.ipAddress}</span>
                                  </>
                                )}
                              </div>
                            </div>
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between pt-4 border-t border-brand-divider">
              ...
              <div className="flex items-center gap-1">
                ...
                <span className="text-[10px] font-mono text-brand-text-muted px-2">{page} / {totalPages}</span>
